...
enrichment<-function(x,adjust,threshold) {
    ...
    {
    result[i,7]<-as.numeric(all[which(all[,1]%in%result[i,1]),3])
    result[i,8]<-as.numeric(length(x))
    result[i,9]<-as.numeric(result[i,4])
    }
    colnames(result)=c("TF","Rank","Annotated_Gene","Annotated_Gene_number","P_Value","FDR","num1","num2","num3")
